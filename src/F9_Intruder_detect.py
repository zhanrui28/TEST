...
def run_alarm_modal():
    ...
    # >>> NEW: turn camera on when alarm starts
    try:
        Phone_noti.setup_camera()
    except Exception as e:
        _dbg(f"Camera start failed: {e}")
    ...
    try:
        ...
        # >>> NEW: stop camera when alarm clears
        try:
            Phone_noti.stop_camera()
        except Exception as e:
            _dbg(f"Camera stop failed: {e}")
